 ...
 *  Lookup table based for speed optimization
 */
...
bool crc16_verify(const uint8_t* data, uint16_t len_with_crc) {
    ...
    /* Compute CRC over data (excluding last 2 CRC bytes) */
    uint16_t computed = crc16_compute(data, len_with_crc - 2);
    
    /* Extract stored CRC (Big Endian) */
    uint16_t crc_offset = len_with_crc - 2;
    uint16_t stored = (uint16_t)((data[crc_offset] << 8) | data[crc_offset + 1]);
    
    return (computed == stored);
}

void crc16_append(uint8_t* data, uint16_t len_without_crc) {
    uint16_t crc = crc16_compute(data, len_without_crc);
    
    /* Append CRC in Big Endian format */
    data[len_without_crc] = (uint8_t)(crc >> 8);
    data[len_without_crc + 1] = (uint8_t)(crc & 0xFF);
}
